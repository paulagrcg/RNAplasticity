...
def evodyn(rhogND,evgND,entropy,targets,initpop, seqscommon,extreme0, extreme1, samplenum = 1,plasticoption = False, gengap = 10, L=12, gpmap = {}, mu = 0.05, generations = 10000, Npop = 100, fitlands = 2):
        ...
        for i in range(1,generations+1): # i is generation number
                ...
                alphalist[i-1] = float(len(freq.keys())/Npop)
...
if __name__ == "__main__":
        ...
        mu = float(sys.argv[7])
        initoption = int(sys.argv[8]) #extremeoption -> 0: low plasticity genotypes
        extremeoption = int(sys.argv[9]) #e.g. if we want to start simulation with target 0, the initial population should be at extreme of target 1
        Npop = int(sys.argv[10])
        samples = int(sys.argv[11])
        run = int(sys.argv[12])#out of 9 (0-8)
        run = int((run/800.)*8) 
        ...
        path1 = path + pairphenopath +"/"
        

        with open("/rds/user/pg520/hpc-work/dictRNA12tot.pkl","rb") as f: gpmap = pickle.load(f)

        seq_soft_extreme_0 = np.load(path1 + "seqssoftextreme0.npy")
        seq_soft_extreme_1 = np.load(path1 + "seqssoftextreme1.npy")
        seqstrueextreme0 = stringtoint(np.load(path1 + "seqstrueextreme0.npy"))
        seqstrueextreme1 = stringtoint(np.load(path1 + "seqstrueextreme1.npy"))
        seqscommon = stringtoint(np.load(path1 + "seqscommon.npy"))
        ...
        for sample in range(0,samples): 
                ...
                dictsample = {'meanfitness': list(meanfitness.values()),'seqscommon': seqscommonlist, 'extreme0': extreme0list, 'extreme1': extreme1list, 'other':otherlist,'meanprobs0seqscom':meanprobs0seqscom, 'meanprobs1seqscom':meanprobs1seqscom, 'meanprobs0': meanprobs0, 'meanprobs1': meanprobs1, 'alphalist': alphalist}
        ...
        dfstd.to_csv(path1 + 'stddataprobsalphadata_'+"fitlands"+str(fitlands)+"_initoption"+str(initoption)+"_extremeoption"+str(extremeoption)+"_samplenum"+str(samplenum)+"_plasticoption"+str(plasticoption)+"_gengap"+str(gengap)+"_mu"+str(mu)+"_generations"+str(generations)+"_Npop"+str(Npop)+'.csv')
